...
function attn_box_plugin_options() {
	...
	if (!empty($wp_attention_box_upgrade_to_6)) {
		...
		<span id="ft_message_title">Looks like you just upgraded to <strong>0.6.x</strong> </span> <a id="close_upgrade_message" href="#"><strong>Close</strong></a><br><br>
		Since you have 6 empty DIV settings, you can pre-fill those with "starter styles", using the buttons at the very bottom of this Settings Page.
		<br><br>( Note: Make sure to use the Prefill button on the Left if you wish to Keep your current DIV settings for the first 4! )
		</div>
        ...
        <tr>    
            ...
            <td>
                 ...
                 <select id="bstyle<?php echo $idx;?>" class="border_style"  name="attnbox_options[bstyle<?php echo $idx;?>]">
                    <option value='solid' <?php selected('solid', $options['bstyle' . $idx]); ?>>Solid</option>
                    <option value='dotted' <?php selected('dotted', $options['bstyle' . $idx]); ?>>Dotted</option>
                    <option value='dashed' <?php selected('dashed', $options['bstyle' . $idx]); ?>>Dashed</option>
                    <option value='double' <?php selected('double', $options['bstyle' . $idx]); ?>>Double</option>
                    <option value='groove' <?php selected('groove', $options['bstyle' . $idx]); ?>>Groove</option>
                    <option value='ridge' <?php selected('ridge', $options['bstyle' . $idx]); ?>>Ridge</option>
                    <option value='inset' <?php selected('inset', $options['bstyle' . $idx]); ?>>Inset</option>
                    <option value='outset' <?php selected('outset', $options['bstyle' . $idx]); ?>>Outset</option>
                  </select>
          ...
          <tr>    
            <th scope="row"><input id="round<?php echo $idx;?>" type="checkbox" name="attnbox_options[enable_rounded<?php echo $idx;?>]" value="1" <?php checked('1', $options['enable_rounded' . $idx]); ?>> Rounded Corners: </th>
          ...
          <tr>
           ...
           <th>Text Alignment</th>
            <td>Left Align: <input type="radio"  <?php checked('left', $options['align' . $idx]); ?> name="attnbox_options[align<?php echo $idx;?>]" value="left" > 
                Center Align: <input type="radio" <?php checked('center', $options['align' . $idx]); ?> name="attnbox_options[align<?php echo $idx;?>]" value="center"></td>
          </tr>
    ...
    <div>
      <p>If you've just upgraded to <strong>0.6.x</strong> and you ... <br> ...<span>Don't Want to Overwrite</span> your existing first four DIV's</p>
        <input title="You will need to Click 'Save Changes' after" class="set_div_default button-primary" id="existing_user" type="button" value="Prefill remaining 6 with Defaults">
    </div>
    
     <div>
      <p>If you're new to WP Attention Boxes</p>
        <input title="You will need to Click 'Save Changes' after" id="new_user" class="set_div_default  button-primary" type="button" value="Prefill Defaults for ALL 10 DIV Boxes">
    </div>
